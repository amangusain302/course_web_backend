...
exports.contact = catchAsyncError(async(req, res, next) => {
    ...
    if (!name || !email || !message) return next(new ErrorHandler("All field are mandatory", 400));

    const to = process.env.MY_MAIL;
    const subject = "Contact from CourseWeb";
...
exports.courseRequest = catchAsyncError(async(req, res, next) => {
    ...
    if (!name || !email || !course) return next(new ErrorHandler("All field are mandatory", 400));

    const to = process.env.MY_MAIL;
    const subject = "Request for a course CourseWeb";
    const text = `I am ${name} and my Email is ${email}. \n Requested Course is ${course}`;
